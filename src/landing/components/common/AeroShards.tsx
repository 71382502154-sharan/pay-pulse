...
const resolvePathLength = (aspect: number, weights: readonly number[] | number[]) => {
...
const parseColor = (value?: string, fallback = '#120F17'): number[] => {
...
const resolveQuality = (canvas: HTMLCanvasElement): string => {
...
const resolveDpr = (preset: any, canvas: HTMLCanvasElement): number => {
  const cssPixels = Math.max(1, canvas.clientWidth * canvas.clientHeight);
...
const resolveBloomSize = (size: readonly [number, number] | number[], qualityLevel = 0): readonly [number, number] => {
  const bloomScale = BLOOM_SCALES[qualityLevel] ?? BLOOM_SCALES[0];
  return [Math.max(1, Math.round(size[0] * bloomScale)), Math.max(1, Math.round(size[1] * bloomScale))] as const;
};
...
const configureStyle = (graph: any, settings: any, outputSize: readonly [number, number] | number[], cssSize: readonly [number, number] | number[]) => {
  ...
  graph.styleTarget.resize((mode ? [outputSize[0], outputSize[1]] : [1, 1]) as readonly [number, number]);
  graph.asciiTarget.resize(
    (mode === EFFECTS.ascii
      ? [Math.max(1, Math.ceil(outputSize[0] / cellWidth)), Math.max(1, Math.ceil(outputSize[1] / cellHeight))]
      : [1, 1]) as readonly [number, number]
  );
    ...
    void (async () => {
      try {
        ...
        const resizePostTargets = (qualityLevel = appliedBloomLevel) => {
          ...
          graph.sceneTarget.resize([width, height] as const);
